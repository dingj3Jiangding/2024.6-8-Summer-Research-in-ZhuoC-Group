...
class exp_prcss(size: Int = 8,man_size: Int = 23) extends Module{
	...
	val x_exp_signed = exp_io.x_exp.zext(9)  // Zero extend to 9 bits
  	val y_exp_signed = exp_io.y_exp.zext(9)  // Zero extend to 9 bits
  	val bias_signed = bias.zext(9)       // Zero extend to 9 bits
	...
	.elsewhen (x_isDenormal || y_isDenormal){							//No Inf, NaN or Zero, if contains at least one Denormal.
		...
		{
			val temp_exp = expSum + 1.U
			when(temp_exp > (Fill(exp_io.y_exp.getWidth,"b1".U) - 1.U)){		//Overflow
				exp_io.out_exp := Fill(exp_io.y_exp.getWidth,"b1".U)
				exp_io.out_man := 0.U
				exp_io.isman := true.B
				exp_io.branch_number := 7.U
			}
			.elsewhen(temp_exp < 1.U){										// when underflow, the exp should be set to the min normal exp
				exp_io.out_exp := Fill(exp_io.y_exp.getWidth, "b0".U)
			...
			.otherwise{
				...
				exp_io.out_exp := temp_exp_unsigned()@@
